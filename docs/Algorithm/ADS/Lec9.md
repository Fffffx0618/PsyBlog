...
- 令$c_{ij}$为活动$a_i$和$a_j$之间最多的活动数，则得到状态转移方程
  ...
  $$
  _{ij} = \begin{cases}0 & \text{if}\ S_{ij} = \emptyset \\ \max\limits_{a_k \in S_{ij}} \{c_{ik} + c_{kj} + 1\} & \text{if}\ S_{ij} \ne \emptyset\end{cases}
  $$
...
- 得到正确的贪心算法后，修改DP的状态转移方程：
  ...
  $$
  _{1, j} = \begin{cases}1 & \text{if}\ j = 1 \\ \max \{c_{1, j - 1}, c_{1, k(j)} + 1\} & \text{if}\ j > 1\end{cases}
  $$
