...
!!! note "代码实现"
    ...
    ``c     Step 1  Set i = 1;                 FA = f(a);     Step 2  while (i <= N_max) do steps 3-6             Step 3  Set p = a + (b - a) / 2;  // computer p_i                     FP = f(p);             Step 4  if (FP == 0) or (b - a) / 2 < TOL then Output(p);                     STOP;  // successful             Step 5  Set i++;             Step 6  if sign(FA) * sign(FP) > 0 then Set a = p; FA = FP;                     else set b = p;  // update a_i, b_i     Step 7  Output(Method failed after N_max iterations);  // unsuccessful             Stop.     ``
...
将 $f(x)$ 的根看作 $g(x)$ 的不动点（ $g(x)=f(x)+x$ ），即：

$$
f(x)=0 \stackrel{equivalent} \Leftrightarrow x = g(x)
$$
...
!!! note "Code implement"
    ...
    ``c     Step 1  Set i = 1;     Step 2  while (i <= N_max) do steps 3-6             Step 3  Set p = p_0 - f(p_0) / f'(p_0);  // compute p_i             Step 4  if |p - p_0| < TOL then Output(p);  // successful                 STOP;             Step 5  Set i++;             Step 6  Set p_0 = p;  // update p_0     Step 7  Output(The method failed after N_max iterations);  // unsuccessful     ``
...
- 若 $\alpha = 2$，那么序列是**二次(quadratically)收敛**的
  ...
  - 牛顿法：
    ...
    $$
    = p_n - \underbrace{\frac{f(p_n)}{f'(p_n)}}_{p_{n+1}} - \frac{f''(\xi_n)}{2!f'(p_n)}(p - p_n)^2 \Rightarrow \frac{|p_{n+1} - p|}{|p_n - p|^2} = \frac{f''(\xi_n)}{2f'(p_n)}
    $$
  - 只要 $f' (p)\neq 0$，牛顿法至少是**二次收敛**的
    所以， $\alpha$ *越大*，收敛速度*越快*。
...
$$
p_{n+1} = g(p_n) = \underbrace{g(p)}_{=p} + g'(p)(p_n - p) + \cdots + \underbrace{\frac{g^{(\alpha)}(\xi_n)}{\alpha!}}_{=\lambda \text{ (when } n \to \infty)} (p_n - p)^\alpha
$$
...
$$
p \approx \frac{p_{n+2}p_n - p_{n+1}^2}{p_{n+2} - 2p_{n+1} + p_n} \approx p_n - \frac{(p_{n+1} - p_n)^2}{p_{n+2} - 2p_{n+1} + p_n}
$$

于是，我们可以构造序列 $\{\hat{p}_n\}_{n=0}^{\infty}$，其中

$$
\hat{p}_n = \{\Delta^2\}(p_n)=p_n - \frac{(p_{n+1} - p_n)^2}{p_{n+2} - 2p_{n+1} + p_n} = p_n - \frac{(\Delta p_n)^2}{\Delta^2 p_n}
$$
...
!!! note "Theorem"
    ...
    假设序列 $\{p_n\}(n = 1,2,\ldots)$ 线性收敛到极限 $p$，且对于所有充分大的数 $n$，有 $(p_n - p)(p_{n+1} - p) > 0$。那么序列 $\{\hat{p}_n\}(n = 1,2,\ldots)$ 收敛到 $p$ 的速度快于 $\{p_n\}(n = 1,2,\ldots)$，即：
...
!!! note "Code"
    ...
    ``c     Step 1  Set i = 1;     Step 2  while (i <= N_max) do steps 3-6             Step 3  Set p_1 = g(p_0);                         p_2 = g(p_1);                         p = p_0 - (p_1 - p_0)^2 / (p_2 - 2 * p_1 + p_0);             Step 4  if |p - p_0| < TOL then Output(p);  // successful                 STOP;             Step 5  Set i++;             Step 6  Set p_0 = p;  // update p_0     Step 7  Output(The method failed after N_max iterations);  // unsuccessful     ``
