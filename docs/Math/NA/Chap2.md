...
过程如下图，
<img src="images/image-4.png" alt="image-4" width="256" height="166">
...
- 若 $\alpha = 2$，那么序列是**二次(quadratically)收敛**的
    - 牛顿法：
        ...
        $$
        p = p_n - \underbrace{\frac{f(p_n)}{f'(p_n)}}_{p_{n+1}} - \frac{f''(\xi_n)}{2!f'(p_n)}(p - p_n)^2 \Rightarrow \frac{|p_{n+1} - p|}{|p_n - p|^2} = \frac{f''(\xi_n)}{2f'(p_n)} 
        $$
