# Lec6.Relational Database Design
...
!!! info "Procedure for Computing $F^+$"
    ...
    ``text     开始      ↓     F+ = F（先放入原始依赖）      ↓     ┌─→ 对 F+ 中每个 f，用自反律 + 增广律推导 → 新依赖加入 F+     │   ↓     │  对 F+ 中每对 (f1, f2)，若能用传递律合并 → 新依赖加入 F+     │   ↓     │  F+ 还在变化吗？     │   ├── 是 → 回到开头继续循环     │   └── 否 → 结束，输出 F+     └────────────────┘     ``
...
!!! question "How to get $a^+$"
    ...
    ``    result := a;     while (changes to result) do         for each β → γ in F do             begin                 if β ⊆ result then result := result ∪ γ             end;     a⁺ := result    ``
...
!!! example
    ...
    ``text     repeat      	Use the union rule to replace any dependencies in F      	        like a1 -> b1 and a1 -> b2 with a1 -> b1 b2      	Find a functional dependency a -> b      			with an extraneous attribute either in a or in b      	If an extraneous attribute is found, delete it from a -> b      until F does not change     ``
